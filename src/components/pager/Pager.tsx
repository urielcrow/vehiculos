...
export const Pager = React.memo(({totalRegistrosPorPagina = 30,totalRegistros=0,pageAdyacent = 3,paginaActual = 1,pagina} : Props) => {
    ...
    const totalPages = Math.trunc(Math.ceil(totalRegistros / totalRegistrosPorPagina));
                ...
                <li onClick={()=>goPage( Math.max(pageCurrent - 1,0)  )} className={`${pageCurrent === 1 ? 'disabled' : ''}`} > <i className='fas fa-angle-double-left'></i> </li>
                    {
                        ...
                        pagerInit.map(page=>{
                            ...
                            //Segundo caso <1...234...8>
                                ...
                                if( page < totalPages){
                                    if( page <= pageCurrent + Math.trunc( pageAdyacent / 2) && page >= pageCurrent - Math.trunc( pageAdyacent / 2)  )
                                        return li(page,pageCurrent,goPage);
                                    return <div className= "element-empty" key={page}></div>
                                }
...
const generateIndex = (total : number)=>{
    let temp = [];
...
export const generateLimit = (pag : number,total : number)=>{
    return ( pag > 1) ? pag * total - total : 0;
}

const li = (page : number ,pageCurrent : number,goPage : (pag: number) => void ) => (<li onClick={()=>goPage(page)} className={ page === pageCurrent ? 'active' : ''  } key={page}> { page } </li>)
